...
import { Boundary } from '../utils/classes'
...
export class _Map_ {
  ...
  mapCollisionsArrays (array: number[]): Boundary[] {
    ...
    map.forEach((row: number[], i: number) => {
      row.forEach((value: number, j: number) => {
        if (value === 14759) {
          list.push(new Boundary({ x: j * TILESIZE + this.offset.x, y: i * TILESIZE + this.offset.y }))
        }
      })
    })
    return list
  ...
  checkCollision (
    ...
    return (
      ...
      rect1.position.y + rect1.height >= rect2.position.y) // abajo
  }
}