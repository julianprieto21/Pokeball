...
export function StatsGraphic( { pokemon }: { pokemon: Pokemon } ) { // BUG: Grafico aparece grande y se achica.
    ...
    useEffect(() => {
      ...
      const data = {
        ...
        datasets: [{
          ...
          backgroundColor: typesMap[pokemon.types.primary].color.replace('1)', '0.7)'),
        }]
      }
    ...
    return (
        <>
          <Canvas width={0} height={0} canvasRef={chartRef} className="stats-graphic"/>
        </>
    )
}