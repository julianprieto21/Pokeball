...
import { criticRate, effectChart, typesMap, dialogues } from "../constants";
...
export class Engine {
  ...
  playTurn(allyMove: Move, enemyMove: Move) {
    ...
    this.addAction(() => {
      this.makeMove(first_move, first, second);
    }); // first attacks
    this.addAction(() => {
      if (second.isAlive()) {
        // second is alive?
        ...
        if (first.isAlive()) {
          ...
          if (first.isEnemy) {
            this.addAction(() => {
              this.winExperience(second, first);
            }); // if second is ally, win xp
            this.addAction(() => {
              if (second.isLevelUp) {
                this.levelUp(second); // second level up
                if (second.isEvolution) {
                  this.addAction(() => {
                    this.evolve(second);
                  });
                } // second evolve
        ...
        if (second.isEnemy) {
          // second is enemy?
          this.addAction(() => {
            this.winExperience(first, second);
          }); // first win xp
          this.addAction(() => {
            if (first.isLevelUp) {
              this.levelUp(first); // first level up
              if (first.isEvolution) {
                this.addAction(() => {
                  this.evolve(first);
                });
              } // first evolve
  ...
  makeMove(move: Move, user: Pokemon, target: Pokemon) {
    ...
    if (!isHit() || move.damageClass === "status") {
      this.battle.game.interfaceManager.addDialogue(
        format(dialogues.failAttackDialogue, [user.name])
      );
      return;
    }
    ...
    if (move.damageClass === "physical" || move.damageClass === "special") {
      ...
      this.battle.game.animationManager.animateMove(
        user.mainSprite,
        target.mainSprite,
        xMove,
        healthPercentage
    ...
    this.battle.game.interfaceManager.addDialogue(
      format(dialogues.attackDialogue, [user.name, move.name]) + critText
    );
  }
  ...
  getDamage(move: Move, user: Pokemon, target: Pokemon) {
    ...
    const getBonusEffectiveness = (): { e: number; b: number } => {
      ...
      const e =
        effectChart[typesMap[type].id][typesMap[targetTypes.primary].id];
      const b =
        type === userTypes.primary || type === userTypes.second ? 1.5 : 1;
      return { e, b };
    };
  ...
  faint(pokemon: Pokemon) {
    ...
    this.battle.game.interfaceManager.addDialogue(
      format(dialogues.faintDialogue, [pokemon.name])
    );
  }
  ...
  winExperience(winner: Pokemon, looser: Pokemon) {
    ...
    this.battle.game.interfaceManager.addDialogue(
      format(dialogues.winExperienceDialogue, [winner.name, xp.toString()])
    );
  ...
  retreat(pokemon: Pokemon, win: boolean = false) {
    ...
    if (this.canRetreat() || win) {
      const text = win
        ? format(dialogues.winReatreatDialogue, [pokemon.name])
        : format(dialogues.retreatDialogue, [pokemon.name]);
      ...
      this.battle.game.interfaceManager.addDialogue(
        format(dialogues.failRetreatDialogue, [pokemon.name])
      );
    }
  }
  ...
  changePokemon(pokemon: Pokemon) {
    ...
    this.battle.game.interfaceManager.addDialogue(
      format(dialogues.intro2Dialogue, [pokemon.name])
    );
    this.battle.game.animationManager.changePokemonAnimation(
      pokemon.mainSprite,
      leave
    );
  }
  ...
  levelUp(pokemon: Pokemon) {
    this.battle.game.interfaceManager.addDialogue(
      format(dialogues.levelUpDialogue, [
        pokemon.name,
  ...
  evolve(pokemon: Pokemon) {
    this.battle.game.interfaceManager.addDialogue(
      format(dialogues.evolveDialogue, [
        pokemon.name,
        pokemon.evolutionChain.first!.name,
      ])
    );
  }
}
