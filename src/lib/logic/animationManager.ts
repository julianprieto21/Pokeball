...
export class AnimationManager {
   ...
   */
  allyAnimateEntry() {
    if (!this.allySprite || !this.enemySprite)
  ...
  retreatAnimation(sprite: PokemonSprite) {
    ...
    this.tl.to(sprite.position, {
      ...
      duration: 1 / GAME_SPEED,
      onComplete: () => {
        this.game.interfaceManager.quitBattle();
      },
    });
  }
  ...
  animateExperience(percentage: number) {
    this.tl.to("#ally-current-experience-bar", {
      ...
      onComplete: () => {
        this.game.canClick = true;
        this.game.interfaceManager.getSetters().level!(
          this.game.battle!.ally!.level
        ); // TODO: #5
        if (percentage == 100) {
          window.document.getElementById(
            "ally-current-experience-bar"
          )!.style.width = "0%"; // FIXME: Hacer lo mismo de otra manera
  ...
  changePokemonAnimation(
    entry: PokemonSprite,
    leave: PokemonSprite | boolean = false
  ) {
