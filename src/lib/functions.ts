 ...
 */
export function statValue(data: {
  base: { value: number; effort: number };
  iv: number;
  nature: number;
  level: number;
  isHp: boolean;
  ...
  if (data.isHp) {
    return Math.floor(
      ((stat + effort + data.iv) * data.level) / 100 + data.level + 10
    );
  }
  return Math.floor(
    (((stat + effort + data.iv) * data.level) / 100 + 5) * data.nature
  );
}
  ...
  return {
    ...
    evolution: {
      first: {
        ...
        level: evolution.chain.evolves_to[0]?.evolution_details[0].min_level,
      },
      second: {
    ...
    nature: {
      ...
      name: nature.name,
      decreasedStat:
        natureNameMap[nature.decreased_stat?.name.replace("-", "_")],
      increasedStat:
        natureNameMap[nature.increased_stat?.name.replace("-", "_")],
    },
