...
import { parsePokemonData } from "./functions";
...
export async function getPokemonData(
  ...
  const evolutionChain = speciesData.evolution_chain.url.split(
    "/"
  )[6] as string;
  ...
  const pokemonData: PokemonData = parsePokemonData(
    mainData,
    speciesData,
    evolutionData,
    natureData
